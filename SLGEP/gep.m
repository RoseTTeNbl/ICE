function [A,P,CV,trace,prd,timec] = gep(~,h1,cof1,p1,cofb1,synum,G,group)
...
%   计算目标值和频率
[A,ob,~,~,~]=fun_abl(x,cons,cofb,A,cofb1,D,group);  %fre8x1,obNPx1
trace=ones(G,1);
timec=zeros(G,1);
trace(1,1)=min(ob);
fre=fr(x,fre,1);
...
for gen=1:G
    ...
    [A,obnew,res1,CX,~]=fun_abl(v,cons,cofb,A,cofb1,D,group);
    v(1,:)=best;
    ...
    [A,ob,~,~,covg]=fun_abl(x,cons,cofb,A,cofb1,D,group);
    fre=fr(x,fre,1);
    cfre=fr(x,cfre,2);
