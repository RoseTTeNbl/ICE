function [A,P,CV,trace,prd,timec] = ADFGEP(~,h,h1,h0,cof1,p1,cofb1,synum,G)
...
%   计算目标值和频率
[A,ob,~,~,~]=fun(x,cons,cofb,A,cofb1,D);  %fre8x1,obNPx1
trace=ones(G,1);
timec=zeros(G,1);
trace(1,1)=min(ob);
...
for gen=1:G
    ...
    for m=1:NP
        ...
        for k=1:p
            ...
            PRE(k,:)=[x(r1,1:cut),x(r2,cut+1:size(PRE,2))];
            [~,cpx(k,1)]=obj(PRE(k,:),cofb,g,dt,D);
        end
        [~,bst]=max(cpx);
        tem=PRE(bst,:);
        v(m,:)=tem;
    ...
    [A,obnew,~,~,~]=fun(v,cons,cofb,A,cofb1,D);
    v(1,:)=best;
    ...
    [A,ob,res,CX,covg]=fun(x,cons,cofb,A,cofb1,D);
    fre=fr(x,fre,1);
    cfre=fr(x,cfre,2);
...
trb(gen,1:D+D0)=best;
[A,trv(gen,1),trb(gen,D+D0+1:D+D0+D1)]=MAPconv(covg(ind,1),A,cofb1);
ftem=fun1(trb(gen,D+D0+1:D+D0+D1),cofb1);
...
% prt(trb(gen,D+D0+1:D+D0+D1),3);
    ...
    if abs(covg(ind,1)-ftem)<=1e-4 && compl(trb(gen,:),D)>0 && rand(1)>0.3
        P=[P;trb(gen,:)];
        [P,~,~]=unique(P,'rows');
        CV1=[CV1;trv(gen,1)];
        prd(gen,1)=size(P,1);
